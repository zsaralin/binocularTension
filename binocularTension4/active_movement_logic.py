...
def update_active_movement(general_head_points_transformed, person_moving_status, movement_points_transformed, image_width, image_height, intrinsics):
    ...
    global active_movement, active_movement_type, active_movement_id, isLooking, last_movement_time, last_tracked_time
    ...
    if active_movement_id not in moving_persons:
        isLooking = True
    # Check if the active movement ID has been maintained for longer than extended_timeout
    if active_movement_id and (current_time - last_tracked_time >= extended_timeout):
        # Reset to look for a new active movement, ignoring the current active_movement_id
        ...
        if alternative_persons:
            # Select the closest alternative person if available
            ...
            # Select the closest alternative object if available
            active_movement_id, active_movement = min(alternative_objects.items(), key=lambda item: item[1][2])
            active_movement_type = 'object'
            last_movement_time, last_tracked_time = current_time, current_time
        else:
            # Continue with the current active movement if no alternatives are available
            last_tracked_time = current_time
    # Remaining logic to handle movement updates
    # print(isLooking, active_movement_id)
    if isLooking or active_movement_type is None:
        ...
        elif active_movement_type == 'object' and current_time - last_movement_time >= 5:
