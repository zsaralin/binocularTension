...
from PyQt5.QtWidgets import (
    QWidget, QVBoxLayout, QHBoxLayout, QLabel, QSlider, QPushButton, QLineEdit
)
...
class ControlPanelWidget(QWidget):
    def __init__(self, parent=None):
        ...
        self.divider = [
            ...
            self.config['x_divider_angle'], self.config['z_divider'], self.config['z_divider_curve']
        ]
        self.movement = [
    ...
    def init_ui(self):
        ...
        # Add instructions for key press shortcuts
        instructions_label = QLabel("Shortcuts:\nPress 'P' to Pause/Unpause Point Cloud\nPress 'R' to Reset View")
        instructions_label.setAlignment(Qt.AlignCenter)
        layout.addWidget(instructions_label)

        # Rotation sliders
        layout.addWidget(QLabel("Rotation"))
        self.create_slider_group(layout, "Rot X", 0, -180, 180, self.rotation, 0)
        self.create_slider_group(layout, "Rot Y", 1, -180, 180, self.rotation, 1)
        self.create_slider_group(layout, "Rot Z", 2, -180, 180, self.rotation, 2)

        # Translation sliders
        layout.addWidget(QLabel("Translation"))
        self.create_slider_group(layout, "Trans X", 0, -10, 10, self.translation, 0, 0.1)
        self.create_slider_group(layout, "Trans Y", 1, -10, 10, self.translation, 1, 0.1)
        self.create_slider_group(layout, "Trans Z", 2, -10, 10, self.translation, 2, 0.1)

        # Divider sliders
        layout.addWidget(QLabel("Divider"))
        self.create_slider_group(layout, "Top Y Divider", 0, -5, 15, self.divider, 0, 0.1)
        self.create_slider_group(layout, "Bottom Y Divider", 1, -5, 15, self.divider, 1, 0.1)
        self.create_slider_group(layout, "X Divider Angle", 2, 10, 90, self.divider, 2, 1)
        self.create_slider_group(layout, "Z Divider", 3, -15, 15, self.divider, 3, 0.1)
        self.create_slider_group(layout, "Z Divider Curve", 4, 0, 10, self.divider, 4, 1)

        # Movement detection sliders
        layout.addWidget(QLabel("Movement Detection"))
        self.create_slider_group(layout, "Min Contour Area", 0, 100, 1000, self.movement, 0, 1)
        self.create_slider_group(layout, "Person Movement Threshold", 1, 0.01, 1, self.movement, 1, 0.01)
    ...
    def update_slider_from_input(self, input_field, slider, min_val, max_val, step):
        """Update the slider's value based on the input field's value."""
        try:
            value = float(input_field.text())
            scaled_value = int(value * 10) if step == 0.1 else int(value * 100) if step == 0.01 else int(value)
            if slider.minimum() <= scaled_value <= slider.maximum():
                slider.setValue(scaled_value)
            else:
                input_field.setText(f"{slider.value() / (10 if step == 0.1 else 100 if step == 0.01 else 1):.2f}")
        except ValueError:
            input_field.setText(f"{slider.value() / (10 if step == 0.1 else 100 if step == 0.01 else 1):.2f}")
    ...
    def sync_with_live_config(self):
        """Syncs slider values with live configuration parameters."""
        # Rotation, translation, divider, and movement syncs
