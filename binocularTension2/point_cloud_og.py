...
def grid(size=1, n=10, width=1):
    ...
    batch.draw()
def draw_3d_cube(size=0.5, z_depth=-2.0, color=(0.0, 1.0, 0.0)):
    """Draws a 3D cube centered at a specific depth."""
    half_size = size / 2.0
    gl.glColor3f(*color)  # Set the color for the cube (green in this case)

    # Start drawing the cube using GL_QUADS (quadrilateral primitive)
    gl.glBegin(gl.GL_QUADS)

    # Front face
    gl.glVertex3f(-half_size, -half_size, z_depth + half_size)  # Bottom-left
    gl.glVertex3f(half_size, -half_size, z_depth + half_size)   # Bottom-right
    gl.glVertex3f(half_size, half_size, z_depth + half_size)    # Top-right
    gl.glVertex3f(-half_size, half_size, z_depth + half_size)   # Top-left
    ...
    # Left face
    gl.glVertex3f(-half_size, -half_size, z_depth - half_size)  # Bottom-back
    gl.glVertex3f(-half_size, -half_size, z_depth + half_size)  # Bottom-front
    gl.glVertex3f(-half_size, half_size, z_depth + half_size)   # Top-front
    gl.glVertex3f(-half_size, half_size, z_depth - half_size)   # Top-back
    ...
    # Top face
    gl.glVertex3f(-half_size, half_size, z_depth + half_size)   # Front-left
    gl.glVertex3f(half_size, half_size, z_depth + half_size)    # Front-right
    gl.glVertex3f(half_size, half_size, z_depth - half_size)    # Back-right
    gl.glVertex3f(-half_size, half_size, z_depth - half_size)   # Back-left
...
def on_draw():
    ...
    gl.glEnable(gl.GL_POINT_SMOOTH)  # Enable smooth points

    width, height = window.get_size()
    gl.glViewport(0, 0, width, height)

    # Set up the perspective projection (camera frustum)
    ...
    # Camera looking down z-axis (camera at 0, 0, 0, looking towards 0, 0, -1)
    gl.gluLookAt(0, 0, 0,  0, 0, -1,  0, 1, 0)

    # Translate the scene back to simulate the camera moving forward
    gl.glTranslatef(0, 0, 0)  # Move the point 3 units in front of the camera

    # Set the point size larger and change the color to purple
    gl.glPointSize(10.0)
    gl.glColor3f(1.0, 0.0, 1.0)  # Purple

    # Draw the point in 3D space at the origin of the translated space
    gl.glBegin(gl.GL_POINTS)
    gl.glVertex3f(0, 0,-3 )  # Center the point at (x=0, y=0, z=3)
    gl.glEnd()
    ...
    axes()
    draw_3d_cube(size=0.5, z_depth=6.0, color=(0.0, 1.0, 0.0))  # Green cube
    draw_3d_cube(size=0.5, z_depth=4.0, color=(0.0, 1.0, 0.0))  # Green cube
    draw_3d_cube(size=0.5, z_depth=5.0, color=(0.0, 1.0, 0.0))  # Green cube
    draw_3d_cube(size=0.5, z_depth=3.0, color=(0.0, 1.0, 0.0))  # Green cube
    draw_3d_cube(size=0.5, z_depth=2.0, color=(0.0, 1.0, 0.0))  # Green cube
