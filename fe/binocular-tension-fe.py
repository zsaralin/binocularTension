...
class FullScreenBlinkApp(QWidget):
    ...
    def display_image(self, filename):
        if hasattr(self, 'blink_sleep_manager') and self.blink_sleep_manager.sleep_manager.display_off:
            return
    ...
    def display_black_image(self):
            filename = "./eyeballImages/off.png"
            if os.path.exists(filename):
                pixmap = QPixmap(filename)  # Load the image directly
